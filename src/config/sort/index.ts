...
//   target: any;
// }
interface DargComponentConfigType {
...
const dragComponentConfig: DargComponentConfigType = {
  currentDragComponent: {
    dom: null,
    type: null,
    props: null,
...
class DragOption {
  ...
  public dragStart() {
    ...
    this.sourceDom.addEventListener("dragstart", (event: any) => {
      // 保存被拖动元素的引用 dragging
      dragComponentConfig.currentDragComponent.dom = event.target;
...
class DropOption {
  ...
  public dropEvent(event: any) {
    ...
    if (
      ...
      if (dragComponentConfig.currentDragComponent.dom) {
        const type =
          dragComponentConfig.currentDragComponent.dom.getAttribute(
            "data-type"
          );
        ...
        vDom.setAttribute("data-in-container", "true");
